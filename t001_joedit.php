...
loadjs.ready("head", function() {
	...
	ft001_joedit.lists["x_NoJO"] = <?php echo $t001_jo_edit->NoJO->Lookup->toClientList($t001_jo_edit) ?>;
	ft001_joedit.lists["x_NoJO"].options = <?php echo JsonEncode($t001_jo_edit->NoJO->lookupOptions()) ?>;
	ft001_joedit.autoSuggests["x_NoJO"] = <?php echo json_encode(["data" => "ajax=autosuggest"]) ?>;
...
<?php if ($t001_jo_edit->NoJO->Visible) { // NoJO ?>
	<div id="r_NoJO" class="form-group row">
		<label id="elh_t001_jo_NoJO" class="<?php echo $t001_jo_edit->LeftColumnClass ?>"><?php echo $t001_jo_edit->NoJO->caption() ?><?php echo $t001_jo_edit->NoJO->Required ? $Language->phrase("FieldRequiredIndicator") : "" ?></label>
		<div class="<?php echo $t001_jo_edit->RightColumnClass ?>"><div <?php echo $t001_jo_edit->NoJO->cellAttributes() ?>>
<span id="el_t001_jo_NoJO">
<?php
$onchange = $t001_jo_edit->NoJO->EditAttrs->prepend("onchange", "");
$onchange = ($onchange) ? ' onchange="' . JsEncode($onchange) . '"' : '';
$t001_jo_edit->NoJO->EditAttrs["onchange"] = "";
?>
<span id="as_x_NoJO">
	<div class="input-group">
		<input type="text" class="form-control" name="sv_x_NoJO" id="sv_x_NoJO" value="<?php echo RemoveHtml($t001_jo_edit->NoJO->EditValue) ?>" size="15" maxlength="25" placeholder="<?php echo HtmlEncode($t001_jo_edit->NoJO->getPlaceHolder()) ?>" data-placeholder="<?php echo HtmlEncode($t001_jo_edit->NoJO->getPlaceHolder()) ?>"<?php echo $t001_jo_edit->NoJO->editAttributes() ?>>
		<div class="input-group-append">
			<button type="button" title="<?php echo HtmlEncode(str_replace("%s", RemoveHtml($t001_jo_edit->NoJO->caption()), $Language->phrase("LookupLink", TRUE))) ?>" onclick="ew.modalLookupShow({lnk:this,el:'x_NoJO',m:0,n:10,srch:true});" class="ew-lookup-btn btn btn-default"<?php echo ($t001_jo_edit->NoJO->ReadOnly || $t001_jo_edit->NoJO->Disabled) ? " disabled" : "" ?>><i class="fas fa-search ew-icon"></i></button>
		</div>
	</div>
</span>
<input type="hidden" data-table="t001_jo" data-field="x_NoJO" data-multiple="0" data-lookup="1" data-value-separator="<?php echo $t001_jo_edit->NoJO->displayValueSeparatorAttribute() ?>" name="x_NoJO" id="x_NoJO" value="<?php echo HtmlEncode($t001_jo_edit->NoJO->CurrentValue) ?>"<?php echo $onchange ?>>
<script>
loadjs.ready(["ft001_joedit"], function() {
	ft001_joedit.createAutoSuggest({"id":"x_NoJO","forceSelect":false});
});
</script>
<?php echo $t001_jo_edit->NoJO->Lookup->getParamTag($t001_jo_edit, "p_x_NoJO") ?>
</span>
...
<?php
	if (in_array("t102_mutasi", explode(",", $t001_jo->getCurrentDetailTable())) && $t102_mutasi->DetailEdit) {
?>
